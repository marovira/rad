...
namespace rad
{
    ...
    template<typename ImageProcessFun>
    void process_images(std::string const& root, std::vector<std::string> const& samples, ImageProcessFun&& fun)
    {
    ...
    void process_images_parallel(std::string const& root, ImageProcessFun&& fun)
    {
        ...
        tbb::parallel_for_each(fs::begin(ite), fs::end(ite), [fun](fs::directory_entry const& entry) {
            auto [filename, img] = load_image(entry.path().string());
            fun(filename, img);
        });
    }
    ...
    template<typename ImageProcessFun>
    void
    process_images_parallel(std::string const& root, std::vector<std::string> const& samples, ImageProcessFun&& fun)
    {
        tbb::parallel_for_each(samples.begin(), samples.end(), [fun, root](std::string const& sample) {
            std::string path     = root + sample;
            auto [filename, img] = load_image(path);
            fun(filename, img);
        });
